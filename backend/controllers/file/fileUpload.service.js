...
const fileFilter = (req, file, cb) => {
  ...
  const allowedTypes = [
    ...
    'application/octet-stream', // STL files sometimes come as this
  ...
  const allowedExtensions = ['.stl', '.ply', '.dcm', '.dicom', '.zip', '.rar', '.7z'];
  
  if (allowedTypes.includes(file.mimetype) || allowedExtensions.includes(ext)) {
    cb(null, true);
  } else {
    cb(new Error(`Invalid file type. Allowed types: images, PDF, DICOM, STL, PLY, ZIP. Got: ${file.mimetype}`), false);
  }
};
...
const determineFileCategory = (file) => {
  ...
  // DICOM files
  if (mime.includes('dicom') || ext === '.dcm') {
    return 'dicom';
  }
