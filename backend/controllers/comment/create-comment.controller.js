...
export const createCaseComment = async (req, res) => {
  try {
    ...
    console.log("Creating comment - User role:", req.user.role); // DEBUG
    ...
    // Create comment data with authorType
    const commentData = {
      caseId,
      ...
      authorType: req.user.role, // REQUIRED - must be set
    ...
    if (req.user.role === "client") {
      ...
      commentData.commentedByDesignerId = null;
    } else if (req.user.role === "designer") {
      commentData.commentedByDesignerId = req.user.profile.id;
      commentData.commentedByClientId = null;
    }

    console.log("Comment data to be created:", JSON.stringify(commentData, null, 2)); // DEBUG
